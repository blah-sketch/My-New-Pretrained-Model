...
def main():
    ...
    # 🐛 BUG FIX: DDP-safe shuffling
    # Each rank needs a different seed for shuffling the dataset, otherwise
    # all ranks would get the exact same data for each batch.
    # We add the rank to the seed to ensure data diversity. `max(0, local_rank)`
    # handles the case where `local_rank` is -1 for single-GPU training.
